...
export class SendGridProvider {
  ...
  async sendEmail(emailData: EmailQueueData): Promise<EmailSendResult> {
    ...
    try {
      ...
      return {
        success: false,
        error: (error as Error).message || 'Failed to send email via SendGrid',
        provider: 'sendgrid' as EmailProvider,
      };
    }
  }
  ...
  async sendTemplateEmail(
    ...
    try {
      ...
      return {
        success: false,
        error: (error as Error).message || 'Failed to send template email via SendGrid',
        provider: 'sendgrid' as EmailProvider,
      };
    }
  }
  ...
  /**
   * Verify webhook signature using ECDSA (SendGrid's method)
   */
  verifyWebhookSignature(
    ...
    try {
      ...
      // Create the payload to verify (timestamp + payload)
      ...
      verify.update(payloadToVerify, 'utf8');
      verify.end();
      
      // Verify the signature using the public key
      return verify.verify(this.config.webhookPublicKey, decodedSignature);
    } catch (error) {
