...
export class SESProvider {
  ...
  private ses: unknown; // AWS SES SDK instance
  ...
  async sendEmail(emailData: EmailQueueData): Promise<EmailSendResult> {
    ...
    try {
      ...
      const result = await (this.ses as any).sendEmail(params).promise();
      ...
      return {
        success: false,
        error: (error as Error).message || 'Failed to send email via AWS SES',
        provider: 'ses' as EmailProvider,
      };
    }
  }
  ...
  async sendTemplateEmail(
    ...
    try {
      ...
      const result = await (this.ses as any).sendTemplatedEmail(params).promise();
      ...
      return {
        success: false,
        error: (error as Error).message || 'Failed to send template email via AWS SES',
        provider: 'ses' as EmailProvider,
      };
    }
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      const result = await (this.ses as any).getSendQuota().promise();
      return result !== null;
    } catch (error) {
   ...
   */
  async getDeliveryStats(_startDate: string, _endDate: string): Promise<any> {
    try {
      if (!this.isInitialized) {
        await this.initialize();
      }
      ...
      const result = await (this.ses as any).getSendStatistics().promise();
  ...
  async getSendingQuota(): Promise<any> {
    try {
      ...
      return await (this.ses as any).getSendQuota().promise();
    } catch (error) {
  ...
  async verifyEmailAddress(email: string): Promise<boolean> {
    try {
      ...
      await (this.ses as any).verifyEmailIdentity({ EmailAddress: email }).promise();
      return true;
    } catch (error) {
  ...
  async listVerifiedEmails(): Promise<string[]> {
    try {
      ...
      const result = await (this.ses as any).listVerifiedEmailAddresses().promise();
