...
export class EmailService {
  ...
  protected templateEngine = new EmailTemplateEngine();
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      console.error('Failed to initialize EmailService:', 'Unknown error');
  ...
  private async loadProviderConfigurations(): Promise<void> {
    try {
      ...
      if (error) {
        console.error('Failed to load email configurations:', "Unknown error");
        return;
      }
      ...
      console.error('Error loading provider configurations:', 'Unknown error');
    }
  }
  ...
  async sendEmail(request: SendEmailRequest): Promise<SendEmailResponse> {
    ...
    try {
      ...
      console.error('Send email error:', "Unknown error");
      return {
        success: false,
  ...
  private async sendEmailImmediate(emailData: EmailQueueData): Promise<SendEmailResponse> {
    try {
      ...
      console.error('Immediate email send error:', "Unknown error");
      return {
        success: false,
        error: (error as Error).message || 'Failed to send email immediately',
      };
    }
  }
  ...
  private async scheduleEmail(
    ...
    try {
      ...
      console.error('Schedule email error:', "Unknown error");
      return {
        success: false,
        error: (error as Error).message || 'Failed to schedule email',
      };
    }
  }
  ...
  async processEmailQueue(): Promise<void> {
    try {
      ...
      if (error) {
        console.error('Failed to process email queue:', "Unknown error");
        return;
      }
      ...
      for (const item of queueItems || []) {
        try {
          ...
          console.error(`Failed to process queue item ${item.id}:`, "Unknown error");
      ...
      console.error('Email queue processing error:', 'Unknown error');
    }
  }
  ...
  private async recordEmailNotification(
    ...
    try {
      ...
      if (error) {
        console.error('Failed to record email notification:', "Unknown error");
      }
    } catch {
      console.error('Record email notification error:', 'Unknown error');
    }
  }
    ...
    if (request.document_id) {
      ...
      if (document) {
        context.document = {
          ...
          sender_name: document.profiles?.[0]?.full_name || '',
          sender_email: document.profiles?.[0]?.email || '',
        };
      }
    }
  ...
  private async getUserEmailPreferences(email: string): Promise<UserEmailPreferences | null> {
    try {
      ...
      console.error('Get user preferences error:', 'Unknown error');
      return null;
    }
  }
  ...
  async handleWebhookEvent(
    ...
    try {
      ...
      if (error) {
        console.error('Failed to update email notification:', "Unknown error");
      }

      // Handle bounces and complaints
      if (event.event === 'bounced' || event.event === 'complained') {
        await this.addToBlacklist(event.email, event.event as 'bounced' | 'complained', event.reason);
      }

      // Update analytics
      await this.updateEmailAnalytics(event);
    } catch {
      console.error('Webhook event handling error:', "Unknown error");
    }
  }
  ...
  private async addToBlacklist(
    ...
    try {
      ...
      if (error) {
        console.error('Failed to add email to blacklist:', "Unknown error");
      }
    } catch {
      console.error('Add to blacklist error:', "Unknown error");
    }
  }
   ...
   */
  private async updateEmailAnalytics(_event: EmailWebhookEvent): Promise<void> {
    try {
      ...
      console.error('Update analytics error:', "Unknown error");
    }
  }
  ...
  async getEmailAnalytics(
    ...
    try {
      ...
      console.error('Get email analytics error:', "Unknown error");
      return [];
    }
  }
  ...
  async processScheduledReminders(): Promise<void> {
    try {
      ...
      if (error) {
        console.error('Failed to process scheduled reminders:', "Unknown error");
        return;
      }
      ...
      for (const reminder of reminders || []) {
        try {
          ...
          console.error(`Failed to send reminder ${reminder.id}:`, "Unknown error");
        }
      }
    } catch {
      console.error('Process scheduled reminders error:', "Unknown error");
    }
  }
  ...
  async testProviderConnection(provider: EmailProvider): Promise<boolean> {
    try {
      ...
      console.error(`Test ${provider} connection error:`, "Unknown error");
      return false;
    }
  }
  ...
  async sendDocumentExpired(data: {
    ...
    try {
      // const template = await this.templateEngine.loadTemplate('document_expired', 'en-US');
      
      const emailData = {
        to: data.recipientEmail,
        email_type: 'document_expired' as const,
        template_variables: {
          recipient_name: data.recipientName,
          document_title: data.documentTitle,
          expired_at: data.expiredAt.toLocaleDateString(),
          document_url: `${emailConfig.appUrl}/documents/${data.documentId}`,
      ...
      console.error('Error sending document expired notification:', "Unknown error");
      throw new Error("Unknown error");
    }
  }
  ...
  async sendSignatureDeclined(data: {
    ...
    try {
      // const template = await this.templateEngine.loadTemplate('document_rejected', 'en-US');
      
      const emailData = {
        to: data.recipientEmail,
        email_type: 'document_rejected' as const,
        template_variables: {
          recipient_name: data.recipientName,
          document_title: data.documentTitle,
      ...
      console.error('Error sending signature declined notification:', "Unknown error");
      throw new Error("Unknown error");
    }
  }
  ...
  /**
   * Get provider instance for webhook processing
   */
  getProvider(providerName: EmailProvider): any {
