...
export class EmailTemplateEngine {
  ...
  async loadTemplate(
    ...
    try {
      ...
      console.error('Failed to load template:', "Unknown error");
    }
  ...
  async processTemplate(
    ...
    try {
      ...
      console.error('Template processing error:', "Unknown error");
      return null;
    }
  }
  ...
  private processConditionalBlocks(template: string, context: TemplateContext): string {
    ...
    const unlessRegex = /\{\{#unless\s+([^}]+)\}\}([sS]*?)\{\{\/unless\}\}/g;
    ...
    const ifElseRegex = /\{\{#if\s+([^}]+)\}\}([sS]*?)\{\{else\}\}([sS]*?)\{\{\/if\}\}/g;
  ...
  private processLoops(template: string, context: TemplateContext): string {
    ...
    const eachRegex = /\{\{#each\s+([^}]+)\}\}([sS]*?)\{\{\/each\}\}/g;
   ...
   */
  private extractUsedVariables(template: EmailTemplate, _context: TemplateContext): string[] {
