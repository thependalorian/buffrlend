...
export function useEmailNotifications(options: UseEmailNotificationsOptions = {}) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to fetch email notifications');
    } finally {
      setLoading(false);
    }
  ...
  const sendEmail = useCallback(async (request: SendEmailRequest): Promise<SendEmailResponse> => {
    ...
    try {
      ...
      const errorMessage = (err as Error).message || 'Failed to send email';
  ...
  const getEmailStats = useCallback(async (documentId?: string) => {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to fetch email statistics');
      return null;
    } finally {
      setLoading(false);
    }
  ...
  const retryEmail = useCallback(async (notificationId: string): Promise<boolean> => {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to retry email');
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const cancelEmail = useCallback(async (notificationId: string): Promise<boolean> => {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to cancel email');
      return false;
    } finally {
      setLoading(false);
    }
