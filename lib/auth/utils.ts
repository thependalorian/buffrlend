/**
 * Authentication Utilities for Buffr Applications
 * 
...
export function validateEmail(email: string): ValidationError | null {
  const rules = validationRules().email as { pattern: RegExp; message: string };
...
export function validatePassword(password: string): ValidationError | null {
  const rules = validationRules().password as { minLength: number; pattern: RegExp; message: string };
...
export function validatePhone(phone: string): ValidationError | null {
  const rules = validationRules().phone as { pattern: RegExp; message: string };
...
export function validateName(name: string, fieldName: string): ValidationError | null {
  const rules = validationRules().name as { minLength: number; maxLength: number; pattern: RegExp; message: string };
...
export function shouldRefreshToken(expiresAt: number): boolean {
  ...
  return timeUntilExpiry <= (securityConfig() as { tokenRefreshThreshold: number }).tokenRefreshThreshold;
}
...
/**
 * Checks if user has unknown admin permissions
 */
export function hasAnyAdminPermissions(user: UserProfile): boolean {
  return (
    user.permissions.can_manage_users ||
    user.permissions.can_manage_loans ||
    user.permissions.can_manage_compliance ||
...
/**
 * Checks if user has unknown of the specified permissions
 */
