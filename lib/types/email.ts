...
export interface SendGridConfig extends EmailProviderConfig {
  ...
  webhookPublicKey?: string; // ECDSA public key for webhook verification
}
...
export type EmailTemplateType<T extends EmailType> = 
  T extends 'loan_application_confirmation' ? 'Loan Application Confirmation' :
  T extends 'loan_approval' ? 'Loan Approval' :
  T extends 'loan_rejection' ? 'Loan Rejection' :
  T extends 'partner_summary' ? 'Partner Summary' :
  T extends 'employee_loan_confirmation' ? 'Employee Loan Confirmation' :
  T extends 'salary_deduction_authorization' ? 'Salary Deduction Authorization' :
  T extends 'payment_reminder' ? 'Payment Reminder' :
...
export const EMAIL_TYPES: Record<EmailType, string> = {
  loan_application_confirmation: 'Loan Application Confirmation',
  loan_approval: 'Loan Approval',
  loan_rejection: 'Loan Rejection',
  partner_summary: 'Partner Summary',
  employee_loan_confirmation: 'Employee Loan Confirmation',
...
export interface ManualEmailRequest {
  ...
  email_type: EmailType;
  recipients: {
    type: 'custom' | 'partners' | 'employees' | 'all_users';
    ids?: string[];
    emails?: string[];
    partnerId?: string;
  };
...
export interface EmployeeLoanConfirmationData {
  employeeName: string;
...
export interface SalaryDeductionAuthorizationData {
  partnerName: string;
  partnerEmail: string;
  employeeName: string;
  employeeEmail: string;
...
export const CONFLICT_TYPES: Record<ConflictType, string> = {
  ...
  partner_workflow: 'Partner Workflow Conflict',
};
