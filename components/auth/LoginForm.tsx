/**
 * Login Form Component for Buffr Applications
 * 
...
import { validateLoginCredentials, formatValidationErrors } from '@/lib/auth/utils';
...
export function LoginForm({ 
  ...
  const { signIn, signInWithGoogle, signInWithWhatsApp, loading } = useAuth();
  ...
  const handleWhatsAppSignIn = async () => {
    try {
      setIsSubmitting(true);
      const result = await signInWithWhatsApp();
      
      if (result.error) {
        const errorMessage = typeof result.error === 'string' ? result.error : 'WhatsApp sign in failed';
        setErrors([errorMessage]);
        onError?.(errorMessage);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'WhatsApp sign in failed';
      setErrors([errorMessage]);
      onError?.(errorMessage);
    } finally {
      setIsSubmitting(false);
