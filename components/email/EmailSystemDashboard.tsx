...
export const EmailSystemDashboard: React.FC<EmailSystemDashboardProps> = ({
  ...
  const { preferences: _preferences, loading: preferencesLoading, error: preferencesError } = useEmailPreferences();
  ...
  const getSummaryStats = () => {
    ...
    const totalEmails = analytics.reduce((sum: number, item: any) => sum + (item.total_sent || 0), 0);
    const deliveredEmails = analytics.reduce((sum: number, item: any) => sum + (item.total_delivered || 0), 0);
    const openedEmails = analytics.reduce((sum: number, item: any) => sum + (item.total_opened || 0), 0);
    const clickedEmails = analytics.reduce((sum: number, item: any) => sum + (item.total_clicked || 0), 0);
    const bouncedEmails = analytics.reduce((sum: number, item: any) => sum + (item.total_bounced || 0), 0);
