/**
 * Admin Manual Email API Route
 * 
 * API endpoint for admin manual email sending with conflict prevention
...
export async function POST(request: NextRequest) {
  try {
    ...
    const adminService = new AdminEmailControlsService();
    ...
    console.error('Admin manual email API error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Failed to process admin email request';
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Admin manual email GET API error:', error);
    const errorMessage = error instanceof Error ? error.message : 'Failed to fetch admin email data';
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
